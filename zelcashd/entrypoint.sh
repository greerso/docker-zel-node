...
# Must have a zelcash config file
if [ ! -f "/mnt/zelcash/config/zelcash.conf" ]; then
  echo "No config found. Exiting."
  exit 1
else
  if [ ! -L $HOME/.zelcash ]; then
    ln -s /mnt/zelcash/config $HOME/.zelcash > /dev/null 2>&1 || true
  fi
fi

# zcash-params can be symlinked in from an external volume or created locally
if [ -d "/mnt/zelcash/zcash-params" ]; then
  if [ ! -L $HOME/.zcash-params ]; then
    echo "Symlinking external zcash-params volume..."
    ln -s /mnt/zelcash/zcash-params $HOME/.zcash-params > /dev/null 2>&1 || true
  fi
else
...
if [ ! -d "/mnt/zelcash/data" ]; then
  echo "Using local data folder"
  mkdir -p /mnt/zelcash/data > /dev/null 2>&1 || true
else
